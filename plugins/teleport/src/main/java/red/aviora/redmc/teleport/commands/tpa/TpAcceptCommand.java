...
public class TpAcceptCommand implements Command<CommandSourceStack> {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        TeleportRequest pending = plugin.getTpaManager().getIncoming(player.getUniqueId());
        if (pending == null) {
            throw new SimpleCommandExceptionType(
                MessageComponentSerializer.message().serialize(
        ...
        TeleportRequest request = plugin.getTpaManager().accept(player);
        if (request == null) {
            throw new SimpleCommandExceptionType(
                MessageComponentSerializer.message().serialize(
                    ApiUtils.formatText(locale.getMessage(player, "tpa.no-request"),
        ...
        ApiUtils.sendPlayerMessageArgs(player,
            locale.getMessage(player, "tpa.accept-success"),
            "%prefix%", locale.getMessage(player, "prefix"));

        Player requester = Bukkit.getPlayer(request.getRequesterUuid());
        if (requester != null && requester.isOnline()) {
            ApiUtils.sendPlayerMessageArgs(requester,
                locale.getMessage(requester, "tpa.request-accepted"),
                "%prefix%", locale.getMessage(requester, "prefix"),
                "%player%", player.getName());

            if (request.isToRequester()) {
                player.teleportAsync(requester.getLocation()).thenAccept(success -> {
                    if (success) {
                        ApiUtils.sendPlayerMessageArgs(player,
                            locale.getMessage(player, "tpa.teleport-success"),
                            "%prefix%", locale.getMessage(player, "prefix"),
                ...
                requester.teleportAsync(player.getLocation()).thenAccept(success -> {
                    if (success) {
                        ApiUtils.sendPlayerMessageArgs(requester,
                            locale.getMessage(requester, "tpa.teleport-success"),
                            "%prefix%", locale.getMessage(requester, "prefix"),
                            "%player%", player.getName());
                    }
                });
