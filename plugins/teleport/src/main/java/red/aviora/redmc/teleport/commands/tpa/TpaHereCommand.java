...
public class TpaHereCommand implements Command<CommandSourceStack> {
    ...
    public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        TeleportRequest request = plugin.getTpaManager().sendRequest(player, target, true);

        ApiUtils.sendPlayerMessageArgs(player,
            locale.getMessage(player, "tpa.request-here-sent"),
            "%prefix%", locale.getMessage(player, "prefix"),
            "%player%", target.getName());

        ApiUtils.sendPlayerMessageArgs(target,
            locale.getMessage(target, "tpa.request-here-received"),
