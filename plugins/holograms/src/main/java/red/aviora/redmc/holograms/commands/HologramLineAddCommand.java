...
public class HologramLineAddCommand implements Command<CommandSourceStack> {
	...
	public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		data.getLines().add(text);
		manager.refreshLines(data);

		ApiUtils.sendCommandSenderMessageArgs(sender, locale.getMessage(sender, "hologram-line-added"),
			"%prefix%", locale.getMessage(sender, "prefix"),
			"%id%", id);
