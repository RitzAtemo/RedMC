...
public class HologramLineSetCommand implements Command<CommandSourceStack> {
	...
	public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		data.getLines().set(index, text);
		manager.refreshLines(data);

		ApiUtils.sendCommandSenderMessageArgs(sender, locale.getMessage(sender, "hologram-line-set"),
			"%prefix%", locale.getMessage(sender, "prefix"),
			"%id%", id,
			"%index%", String.valueOf(index));
