...
public class InvseeAsOfflineCommand implements Command<CommandSourceStack> {
	...
	public int run(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
		...
		String titleRaw = onlineTarget != null
			? VaultPlugin.resolvePlayer(plugin.getLocaleManager().getMessage(player, "admin.invsee.title"), onlineTarget)
			: plugin.getLocaleManager().getMessage(player, "admin.invsee.title")
		...
		String openedMsg = ApiUtils.formatTextString(plugin.getLocaleManager().getMessage(player, "admin.invsee.opened"),
			"%prefix%", prefix);
