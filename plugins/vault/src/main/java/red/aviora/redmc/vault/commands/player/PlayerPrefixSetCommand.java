...
public class PlayerPrefixSetCommand implements Command<CommandSourceStack> {
	...
	public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		VaultManager vaultMgr = JavaPlugin.getPlugin(VaultPlugin.class).getVaultManager();
		var playerData = vaultMgr.getPlayerByName(playerName);
		if (playerData == null) {
		...
		permissions.removeIf(p -> p.getName().startsWith("vault.prefix."));
		pData.addPermission(new PermissionEntry("vault.prefix." + prefix, weight, true));
