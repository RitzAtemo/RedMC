...
public class GroupPrefixGetCommand implements Command<CommandSourceStack> {
	...
	public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		String prefixValue = VaultMetaResolver.getGroupPrefix(groupId, permissionManager);
		String displayValue = prefixValue.isEmpty() ? "" : prefixValue;

		ApiUtils.sendCommandSenderMessageArgs(sender,
			localeManager.getMessage(sender, "group-prefix-value"),
			"%prefix%", localeManager.getMessage(sender, "prefix"),
			"%value%", displayValue);
