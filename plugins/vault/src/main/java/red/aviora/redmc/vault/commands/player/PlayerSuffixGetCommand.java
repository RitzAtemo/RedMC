...
public class PlayerSuffixGetCommand implements Command<CommandSourceStack> {
	...
	public int run(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		String suffixValue = VaultMetaResolver.getSuffix(playerData.getUuid(), permissionManager);
		String displayValue = suffixValue.isEmpty() ? "" : suffixValue;

		ApiUtils.sendCommandSenderMessageArgs(sender,
			localeManager.getMessage(sender, "player-suffix-value"),
			"%prefix%", localeManager.getMessage(sender, "prefix"),
			"%value%", displayValue);
